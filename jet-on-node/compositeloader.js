...
var compositeloader = module.exports;


var targetProjectRoot = "./" // at design time , this becomes the root directory for the JET application
var sourceProjectRoot = "src/"
var compositesDirectory = "js/jet-composites/"
var targetDirectory = "public/" // at runtime this is public nistead of src
...
//loads the jet-composites.json file
...
compositeloader.updateComposite = async function (compositeName) {
  console.log("Update request for " + compositeName)
  console.log("Full composite registry " + JSON.stringify(compositesRegistry))
  var composite = compositesRegistry.reduce(function (composite, item) {
    if (item.name == compositeName) { return item } else return composite
  }
    , null)
  console.log(`About to process this composite ${JSON.stringify(composite)}`)
  let  r = await installComposite(composite)
  console.log(`Done processing ${JSON.stringify(composite)}`)
}
